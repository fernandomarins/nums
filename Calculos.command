...
def numbers(name, birthday):
    ...
    if len(birthday_no_slash) == 8:
        ...
        day = birthday_no_slash[0:2]
        # get the birthday month
        month = birthday_no_slash[2:4]
        # get the birthday year
        year = birthday_no_slash[4:]
        # calculate the daily work
        daily_work = day + month
        # calculate the daily virtude
        daily_virtude = day + year
        ...
        print('Trabalho diário: ', sum_numbers(daily_work))
        ...
        pinnacle_1 = sum_numbers(day) + sum_numbers(month)
        ...
        pinnacle_2 = sum_numbers(sum_numbers(day) + sum_numbers(year))
        ...
        pinnacle_3 = sum_numbers(sum_numbers(pinnacle_1) + sum_numbers(pinnacle_2))
        ...
        pinnacle_4 = (sum_numbers(sum_numbers(month) + sum_numbers(year)))
        ...
        # print forming cycle
        print('Ciclo formador: de', year, 'até', first_cycle, 'na vibração do', sum_numbers(sum_numbers(month)))
        # print productive cycle
        print('Ciclo produtivo: de', first_cycle + 1, 'até ', second_cycle, 'na vibração do', sum_numbers(sum_numbers(day)))
        # print mature cycle
        print('Ciclo de maturidade:', third_cycle, 'na vibração do', sum_numbers(sum_numbers(sum_numbers(year))))
        
        # it's the reduced day minus the reduced month
        first_challenge = sum_numbers(sum_numbers(day)) - sum_numbers(sum_numbers(month))
        # the reduced month minus the reduced year
        second_challenge = sum_numbers(sum_numbers(month)) - sum_numbers(sum_numbers(sum_numbers(year)))
